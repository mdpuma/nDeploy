...
NGINX_RPM_ITER="14.almalinux10"
OPENSSL_VERSION="4.3.2"
...
yum -y install rpm-build libcurl-devel pcre2-devel git GeoIP-devel
...
fpm -s dir -t rpm -C ../nginx-pkg-64 --vendor "IPHOST" --version ${NGINX_VERSION} --iteration ${NGINX_RPM_ITER} -a $(arch) \
	...
	-d zlib -d openssl -d pcre2 -d libcurl --config-files /etc/nginx \
